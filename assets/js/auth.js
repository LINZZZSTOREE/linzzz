...
signupForm?.addEventListener("submit", async (e) => {
  ...
  try{
    ...
    const data = await apiFetch("/api/register", { method:"POST", body:{ username, password, confirm_password: confirmPassword, email } });
    sessionStorage.setItem("lz_pending_email", email);
    sessionStorage.setItem("lz_otp_cooldown_until", Date.now() + (data.cooldown_seconds || 60) * 1000);
...
if (verifyForm){
  ...
  // Cooldown dari signup awal (kalau baru daftar)
  const cooldownUntil = Number(sessionStorage.getItem("lz_otp_cooldown_until") || 0);
  const remainingNow = Math.ceil((cooldownUntil - Date.now()) / 1000);
  if (remainingNow > 0) startCooldown(remainingNow);
  ...
  resendBtn?.addEventListener("click", async (e) => {
    ...
    try{
      ...
      const data = await apiFetch("/api/resend-otp", { method:"POST", body:{ email: pendingEmail } });
